...
function showOverviewMode() {
    let wrap = d3.textwrap().bounds({ height: 80, width: 80 }).method("tspans");
    ...
    d3.csv("data/data.csv").then(function(data) {
        ...
        const svg = d3.select("#container").append("svg")
            ...
            // .attr("viewBox", `${width / 2 - width / 4} ${height / 2 - height / 4} ${width / 2 + width / 4} ${height / 2 + height / 4}`)
            .attr("class", "graph");
...
function showVideoMode() {
    let wrap = d3.textwrap().bounds({ height: 100, width: 100 }).method("tspans");
...
export { showOverviewMode, showVideoMode };